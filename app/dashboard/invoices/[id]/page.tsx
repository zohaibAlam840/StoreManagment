...
import { invoices, invoiceLines, customers, products, payments, paymentModeLabels } from "@/lib/db/schema";
    ...
    <div>
      ...
      >
        ...
        {receiptRows.length > 1 ? (
          receiptRows.map((r) => (
            <p key={r.id} className="text-right">
              Received ({paymentModeLabels[r.mode]}): {r.amount.toFixed(2)}
            </p>
          ))
        ) : (
