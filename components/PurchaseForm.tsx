...
type LineItem = { productId: number; name: string; unit: string; qty: number; cost: number };
...
export function PurchaseForm({
  ...
  const total = subtotal + freight + loadingUnloading + otherExpenses;
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Freight</span>
          <input
            type="number"
            step="0.01"
            value={freight}
